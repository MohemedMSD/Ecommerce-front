...
import { GuideArchPy, GuidePyInfo } from '../../..';
...
const GuideM = ({Guide, Guides, header, action, closeModal, setGuides, seterrorMessage}) => {
    ...
    const [GuideWorks, setGuideWorks] = useState([]);
  /* 
    the two next states :
  ...
  */
    ...
    useEffect(()=>{
      ...
      (async()=>{
        ...
        if (RunOneTime) {
            ...
            if (action === 'update' || action === 'show' || action === 'raport' && Guide) {
    ...
    const updateGuide = async (e, id) => {
      ...
      try {
        ...
        const res = await Axios.post('/guides/' + id,{
          _method : 'PUT',
          name : Name,
          phone : Phone,
          type : Type,
        ...
        if (res.status == 200) {
          ...
          if (SearchType == 'ArchivePay' && res.data?.data?.last_archive_payement?.id) {

            const checkIfExistsArchive = GuideWorks.filter(item => item.id == last_archive_payement?.id)
          ...
          toast.dismiss()
          toast.success('Guide mise à jour avec succés')
    ...
    <div className="fixed top-0 h-screen right-0 w-full flex justify-center items-center">
      ...
      <div className="bg-white w-[98%] sm:w-[70%] lg:w-[55%] z-50 rounded-lg ">
        ...
        <div className="flex items-center justify-between p-3">
          <h1 className="text-[25px] sm:text-[28px] font-bold">{header}</h1>
        ...
        <div className="flex flex-col gap-3 p-2 max-h-[88vh] hide-scrollbar overflow-y-scroll sm:p-5">
          ...
          <div className="border border-gray-400 p-2 rounded-lg">
            <h1 className=" text-[22px] sm:text-[24px] md:text-[26px] text-primary_text font-bold">
              Informations sur guide :
            </h1>
            <div action="" className="relative flex flex-col gap-4 mt-3 p-2">
              ...
              <div className="flex flex-col">
                <div className="flex  gap-3 items-center">
                    ...
                    <select 
                      disabled={action === 'show'}
                      className={` ${ErrType ? 'border-red-500' : 'border-gray-400'} border w-3/4  rounded-lg py-2 px-3 focus:border-primary_text outline-none`}
                ...
                { ErrType && <p className="text-red-500 font-semibold ml-[25%] px-4 py-1">{ErrType}</p>}
              </div>
              ...
              <div className='p-2 border border-gray-300 rounded-lg'>
                ...
                <h2 className='text-center font-semibold text-[19px]'>Travail et Paiements</h2>
                    ...
                    <div className='flex flex-col border gap-3 items-start justify-between border-gray-500 p-2 mt-2 rounded-lg'>
                          ...
                          <div className='flex flex-col sm:flex-row w-full gap-5'>
                            ...
                            <div className='flex w-full flex-col gap-2 sm:flex-row'>
                              ...
                              <div className='flex w-full items-center gap-2'>
                                <input id='work' type='radio' checked={SearchType == 'works'} value='works' name='type' onChange={(e) => setSearchType((pre) => { setGuideWorks([]); setTotalPrice(0); setPayPrice(0); setcurrent_page(1); return e.target.value})} />
                                <label htmlFor='work'>Travail</label>
                              </div>
  
                              <div className='flex w-full items-center gap-2'>
                                <input id='archive' type='radio' checked={SearchType == 'ArchivePay'} value='ArchivePay' name='type' onChange={(e) => setSearchType((pre) => { setGuideWorks([]); setTotalPrice(0); setcurrent_page(1); setPayPrice(0); return e.target.value})} />
                      ...
                      {
                        (SearchType == 'works') && GuideWorks?.length > 0 && (
                          <div className='overflow-x-scroll hide-scrollbar'> 
                            <GuidePyInfo GuideWorks={GuideWorks}/>
                          </div>
                        )
                      }
      
                      {
                        SearchType == 'ArchivePay' && GuideWorks?.length > 0 && (
                          <div className='overflow-x-scroll hide-scrollbar'> 
                            <GuideArchPy 
                              action={action} 
      ...
      
            {
              GuideWorks && (
                        <div className='p-2 border border-gray-300 flex flex-col gap-3 rounded-lg'>
                          ...
                          <div className='flex flex-col sm:flex-row items-center w-full gap-2 sm:gap-0'>
                            <label className='font-semibold w-full sm:w-1/4'>Montant Paye</label>
                          ...
                          </div>
                          {
                            (action != 'show')  && (
                              <>
                                ...
                                { ErrPayDate && <p className="text-red-500 font-semibold ml-[25%] px-4 py-1">{ErrPayDate}</p>}
                                <div className='flex flex-col sm:flex-row w-full items-center gap-2 sm:gap-0'>
              ...
              {
                action === 'update' && <button disabled={processing} onClick={(e) => updateGuide(e, Guide.id)} className="p-2 mx-auto w-full sm:w-fit font-semibold bg-second text-white rounded-md">Enregistrer</button>
              }
              {
                action === 'show' && 
