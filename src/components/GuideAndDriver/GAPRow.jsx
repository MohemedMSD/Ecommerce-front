...
const GAPRow = ({
    ...
    const update = async(id, oldDuration) => {
      ...
      toast.loading('Le traitement est en cours...')
        ...
        try {
            ...
            const res = await Axios.post('/guides/archive-payements/' + id, {
              _method : 'PUT',
              duration : Duration,
              amount : Amount,
              date : Created_at
            ...
            const UpdatedArray = GuideWorks.map((item, index)=>{
            ...
            if (
              UpdatedArray.filter(item => { return item != null }).length < GuideWorks.length
            ) {
              setcurrent_page(1)
              setCall(true)
            }else{
              setGuideWorks(UpdatedArray)
            }
        ...
        if (response.isConfirmed) {
          ...
          try {
            ...
            if (res.status == 200) {
              ...
              const updatedArray = GuideWorks?.filter(item => item.id != id)
  ...
  return (
    <tr className="border-y border-gray-400 hover:bg-gray-200">
      <td className="py-1 text-nowrap border-x md:border-none border-gray-400 px-2 text-center">
      ...
      <td className="py-1 border-x md:border-none border-gray-400 px-2 text-center">{
        InEdit ? <input type="text" className="border w-[89%] text-center border-gray-500 rounded-lg" onChange={(e) => validate_Price(e.target.value, setAmount)} value={Amount} /> : item?.amount
      }</td>
      <td className="py-1 border-x md:border-none border-gray-400 px-2 text-center">{
        item?.paid_date
      }</td>
      {
        action != 'show' && (
          ...
          <td className="px-2 border-r md:border-none border-gray-400">
          {InEdit ? (
            ...
            <div className="flex gap-3">
              <FaEdit className="cursor-pointer hover:text-second" fontSize={18} onClick={() => prepareToEdit({duration : item.duration, amount : item.amount, paid_date : item.paid_date})} />
