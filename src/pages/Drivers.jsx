...
import {CheffeurM, Loading} from "../components";
  ...
  */
  ...
  useEffect(() => {
    ...
    (async ()=>{
        ...
        if (Call) {
          ...
          getDriversAndGuides('drivers', setDrivers, SearchQuery, currentPage, setCall, seterrorMessage, settotalPages, setIsLoading)
  ...
  const prepareToUpdateOrShow = async(id, action) => {
    ...
    try {
      ...
      const res = await Axios.get('/show-drivers/' + id);
      if (res.status === 200) {
  ...
  const search = (e) => {
    ...
    getDriversAndGuides('drivers', setDrivers, StockSearchQuery, 1, setCall, seterrorMessage, settotalPages, setIsLoading)
    ...
    <div>
      ...
      {ShowModalUpdate && (
        <CheffeurM
          ...
          seterrorMessage={seterrorMessage}
          header="Mise à jour de chauffeur"
          action="update"
          Driver={DriverInfo}
          Drivers={Drivers}
          setDrivers={setDrivers}
